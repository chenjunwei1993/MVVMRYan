...
import com.android.library.util.SizeUtils
...
class DividerLine(mContext: Context) : RecyclerView.ItemDecoration() {
    ...
    private fun drawVertical(c: Canvas, parent: RecyclerView, state: RecyclerView.State) {
        ...
        for (i in 0 until childCount) {
            ...
            val right = if (dividerSize == 0) left + SizeUtils.dp2px(DEFAULT_DIVIDER_SIZE.toFloat()) else left + dividerSize
    ...
    private fun drawHorizontal(c: Canvas, parent: RecyclerView, state: RecyclerView.State) {
        ...
        for (i in 0 until childCount) {
            ...
            val bottom = if (dividerSize == 0) top + SizeUtils.dp2px(DEFAULT_DIVIDER_SIZE.toFloat()) else top + dividerSize
            dividerDrawable?.setBounds(left, top, right, bottom)
            //画上去
            dividerDrawable?.draw(c)
        }
